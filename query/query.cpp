...
float query(const Tree &tree, Metrics &metrics, const std::string func_name)
{
    ...
    if (found_count && cuda_ptr_list.size())
    {
        float fp32sctive_sum = 0;
        ...
        for (const Event &kernel : tree.kernel_list)
        {
                ...
                (*i)->correlation == kernel.correlation)
            {
                ...
                fp32sctive_sum += metrics.lookup(lookup_start, lookup_end) * kernel.duration;
                duration_sum += kernel.duration;
                i++;
            }
        }
        float fp32active_avg = fp32sctive_sum / duration_sum;
